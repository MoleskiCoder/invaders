...
void Intel8080::installInstructions() {
	instructions = {
		...
		/* C */	INS(BIND(rnz), Implied, "RNZ", 5),			INS(BIND(pop_b), Implied, "POP B", 10),		INS(BIND(jnz), Absolute, "JNZ ", 10),		INS(BIND(jmp), Absolute, "JMP ", 10),		UNKNOWN(),									INS(BIND(push_b), Implied, "PUSH B", 11),		INS(BIND(adi), Immediate, "ADI ", 7),		INS(BIND(rst_0), Implied, "RST 0", 11),		UNKNOWN(),									INS(BIND(ret), Implied, "RET", 10),			INS(BIND(jz), Absolute, "JZ ", 10),			UNKNOWN(),									UNKNOWN(),									INS(BIND(call), Absolute, "CALL ", 17),		UNKNOWN(),									INS(BIND(rst_1), Implied, "RST 1", 11),		//	C
